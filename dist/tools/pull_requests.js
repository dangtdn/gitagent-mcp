...
export function registerPullRequestTools(server) {
    ...
    server.registerTool("create_pull_request", {
        ...
        try {
            ...
            return formatTextContent(`✅ PR #${response.data.number} created: ${response.data.html_url}`);
        }
        catch (error) {
            return formatError(error);
        }
    ...
    server.registerTool("update_pull_request", {
        ...
        try {
            ...
            return formatTextContent(`✅ PR #${response.data.number} updated successfully!\n${JSON.stringify({
                number: response.data.number,
                title: response.data.title,
                state: response.data.state,
                html_url: response.data.html_url,
    ...
    server.registerTool("request_reviewers", {
        ...
        try {
            ...
            const requestedReviewers = response.data.requested_reviewers?.map((r) => r.login) || [];
            const requestedTeams = response.data.requested_teams?.map((t) => t.slug) || [];
            return formatTextContent(`✅ Reviewers requested for PR #${pull_number}!\nReviewers: ${requestedReviewers.join(", ") || "None"}\nTeams: ${requestedTeams.join(", ") || "None"}`);
        }
        catch (error) {
            return formatError(error);
        }
    ...
    server.registerTool("merge_pull_request", {
        ...
        try {
            ...
            return formatTextContent(`✅ PR #${pull_number} merged successfully!\nSHA: ${response.data.sha}\nMessage: ${response.data.message}`);
        }
        catch (error) {
            return formatError(error);
        }
    ...
    // Tool: Get PR Files
    server.registerTool("get_pr_files", {
        title: "Get PR Files",
    ...
    // Tool: Get PR Reviews
    server.registerTool("get_pr_reviews", {
    ...
    // Tool: Get PR Commits
    server.registerTool("get_pr_commits", {
        title: "Get PR Commits",
        description: "Get all commits from a specific Pull Request. Returns commit SHA, message, author, and date.",
        inputSchema: {
            owner: z.string().describe("Repository owner"),
            repo: z.string().describe("Repository name"),
            pull_number: z.number().describe("Pull request number"),
        ...
        `;
            ...
            const commits = pr.commits.nodes.map((node) => ({
                sha: node.commit.oid,
