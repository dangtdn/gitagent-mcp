...
export function registerIssueTools(server) {
    ...
    server.registerTool("list_issues", {
        ...
        inputSchema: {
            ...
            assignee: z
                .string()
                .optional()
                .describe("Filter by assignee username. Use '*' for any assignee, 'none' for no assignee"),
    ...
    server.registerTool("create_issue", {
        ...
        try {
            ...
            return formatTextContent(`✅ Issue #${response.data.number} created: ${response.data.html_url}`);
        }
        catch (error) {
            return formatError(error);
        }
    ...
    server.registerTool("list_issue_comments", {
        ...
        description: "List all comments on an issue. Returns comments with author, body, and timestamps.",
        inputSchema: {
            owner: z.string().describe("Repository owner"),
            repo: z.string().describe("Repository name"),
            issue_number: z.number().describe("Issue number"),
    ...
    server.registerTool("get_issue_comment", {
    ...
    server.registerTool("update_issue", {
        ...
        try {
            ...
            return formatTextContent(`✅ Issue #${response.data.number} updated successfully!\n${JSON.stringify({
                number: response.data.number,
                title: response.data.title,
                state: response.data.state,
                html_url: response.data.html_url,
    ...
    server.registerTool("close_issue", {
        ...
        try {
            ...
            return formatTextContent(`✅ Issue #${response.data.number} closed successfully!`);
        }
        catch (error) {
            return formatError(error);
        }
    ...
    server.registerTool("list_issues_with_project_fields", {
        title: "List Issues with Project Fields",
        description: "List issues with GitHub Project v2 field values (e.g., Status, Priority) using GraphQL. Requires assignee filter.",
        inputSchema: {
            owner: z.string().describe("Repository owner"),
            repo: z.string().describe("Repository name"),
            ...
            projectStatus: z
                .string()
                .optional()
                .describe("Filter by project status field value (e.g., 'In Progress', 'Done')"),
            first: z
                .number()
                .optional()
        ...
        `;
            ...
            let issues = result.repository.issues.nodes.map((issue) => {
                const projectFields = {};
            ...
            if (assignee) {
                issues = issues.filter((issue) => issue.assignees.some((a) => a.toLowerCase() === assignee.toLowerCase()));
            }
            if (projectStatus) {
    ...
    server.registerTool("get_issue_linked_prs", {
        title: "Get Issue Linked PRs",
        description: "Get all Pull Requests linked to an issue in the Development section. Returns PRs that reference or are connected to this issue.",
        inputSchema: {
            owner: z.string().describe("Repository owner"),
            repo: z.string().describe("Repository name"),
            issue_number: z.number().describe("Issue number"),
        ...
        `;
            ...
            const prMap = new Map();
            result.repository.issue.timelineItems.nodes.forEach((node) => {
                let pr = null;
    ...
    server.registerTool("get_issue_development_info", {
        title: "Get Issue Development Info",
        description: "Get comprehensive development information for an issue including all linked PRs and their commits. This is useful for tracking work progress and time analysis.",
        inputSchema: {
            owner: z.string().describe("Repository owner"),
            repo: z.string().describe("Repository name"),
            issue_number: z.number().describe("Issue number"),
        ...
        `;
            ...
            issue.timelineItems.nodes.forEach((node) => {
                ...
                if (pr && pr.number && !prMap.has(pr.number)) {
                    const commits = pr.commits?.nodes?.map((n) => ({
            ...
            const linkedPRs = Array.from(prMap.values());
            const totalCommits = linkedPRs.reduce((sum, pr) => sum + pr.commits_count, 0);
            const totalAdditions = linkedPRs.reduce((sum, pr) => sum +
                pr.commits.reduce((s, c) => s + (c.additions || 0), 0), 0);
            const totalDeletions = linkedPRs.reduce((sum, pr) => sum +
                pr.commits.reduce((s, c) => s + (c.deletions || 0), 0), 0);
            return formatJsonContent({
                issue: {
                    number: issue.number,
                    title: issue.title,
